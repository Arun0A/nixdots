...
clientmessage(XEvent *e)
{
	...
	if (cme->message_type == netatom[NetWMState]) {
		...
		|| cme->data.l[2] == netatom[NetWMSticky])
			setsticky(c, (cme->data.l[0] == 1 /* _NET_WM_STATE_ADD    */
				|| (cme->data.l[0] == 2 /* _NET_WM_STATE_TOGGLE */ && !c->issticky)));
...
setsticky(Client *c, int sticky)
{
	if (sticky && !c->issticky) {
		XChangeProperty(dpy, c->win, netatom[NetWMState], XA_ATOM, 32,
			PropModeAppend, (unsigned char *)&netatom[NetWMSticky], 1);
		c->issticky = 1;
	} else if (!sticky && c->issticky) {
		/* Keep any other NetWMState flags intact by only clearing the local flag. */
		c->issticky = 0;
	}
	arrange(c->mon);
}
